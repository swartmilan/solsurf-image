...
def mirror_processor(cf_name, image_folder, date_string):
    import glob
    import rawpy
    import numpy as np
    
    #get images
    images = glob.glob(image_folder + '*.NEF')
    n_images = len(images)
    N = n_images/2.0 #number of spots / because there are two images per spot, one dark and one light
    if not (n_images % 2 == 0):
        print('Must be even number of images in the set - check ' + cf_name)
        return
    ...
    for i in range(0, n_images, 2):
        ...
        with open("F:/Google Drive/Masters/Reflectometer image processing/Master Controller/"+str(cf_name)+"_logfile_v3.txt", "a") as logfile:
